...
Behavior:
- rdn.exe (double-click or no args) → Excitement message + auto-start local private node (if not Xchange) + launch the badass harness dashboard (Streamlit) in browser + show system tray icon with controls for Dashboard, Node, MCP, Quit.
- rdn.exe remember "foo"   → Full CLI functionality
- rdn.exe start            → Same as no args (one-stop launch)
- rdn.exe --help           → CLI help
...
def open_dashboard(dash_port=8501, node_port=8765):
    ...
    Note on ports (this is by design):
      ...
      - Dashboard (the beautiful harness UI with metrics, suggestions, Xchange): localhost:{dash_port}
    """
    print(f"\n[rdn] Starting the badass agnostic harness dashboard (Streamlit on :{dash_port})...")
    print(f"[rdn] Local private node (memory backend) is on :{node_port}")
    print("[rdn] This is your one-stop view: live metrics (token savings, velocity, ship rate, vibe stars),")
...
def create_tray_icon():
    ...
    icon = pystray.Icon("rdn", img, "rdn — Agnostic Harness (one-stop)", menu)
    return icon
...
def main():
    ...
    if no_args or is_start:
        print("\n" + "="*70)
        print("🚀 rdn — THE ONE-STOP AGNOSTIC HARNESS (.exe)")
        print("="*70)
        print("""
...
• Launches the full badass harness dashboard (metrics, token savings, velocity,
  ship rate, vibe stars, workflow suggestions, Xchange controls, etc.)
...
Important ports (this exactly matches your report):
  ...
  • Dashboard (the rich UI/harness with all the excitement + metrics): http://localhost:8501

You saw the dashboard at 8501 (correct — that's the beautiful harness UI).
When you manually went to 8765 you saw the real node ("export node was there") — also correct.
...
""")
        ...
        # Launch the exciting visual dashboard (this is the "one stop" experience)
